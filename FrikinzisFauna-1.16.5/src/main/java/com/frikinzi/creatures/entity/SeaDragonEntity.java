...
public class SeaDragonEntity extends GroupFishBase implements IAnimatable {
    private static final DataParameter<Integer> DATA_VARIANT_ID = EntityDataManager.defineId(SeaDragonEntity.class, DataSerializers.INT);
    private AnimationFactory factory = new AnimationFactory(this);
    public static Map<Integer, TranslationTextComponent> SPECIES_NAMES = ImmutableMap.of(
            1, new TranslationTextComponent("message.creatures.leafydragon"),
            2, new TranslationTextComponent("message.creatures.weedydragon"),
            3, new TranslationTextComponent("message.creatures.rubydragon")
    );
    ...
    public static Map<Integer, TranslationTextComponent> DESCRIPTIONS = ImmutableMap.of(
            1, new TranslationTextComponent("description.creatures.leafydragon"),
            2, new TranslationTextComponent("description.creatures.weedydragon"),
            3, new TranslationTextComponent("description.creatures.rubydragon")
    );

    public SeaDragonEntity(EntityType<? extends SeaDragonEntity> p_i50246_1_, World p_i50246_2_) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event)
    {
        ...
        if (this.getVariant() == 1) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("leafy", true));
            return PlayState.CONTINUE;
        } else {
        event.getController().setAnimation(new AnimationBuilder().addAnimation("others", true));
        return PlayState.CONTINUE;
    }
    }
    ...
    protected ItemStack getBucketItemStack() {
        return new ItemStack(CreaturesItems.SEADRAGON_BUCKET);
    }
    ...
    public float getHatchChance() {
        return Double.valueOf(CreaturesConfig.seadragon_hatch_chance.get()).floatValue();
    }
