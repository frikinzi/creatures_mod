...
public class SawfishEntity extends GroupFishBase implements IAnimatable {
    private static final DataParameter<Integer> DATA_VARIANT_ID = EntityDataManager.defineId(SawfishEntity.class, DataSerializers.INT);
    private static final DataParameter<Integer> GENDER = EntityDataManager.defineId(SawfishEntity.class, DataSerializers.INT);
    ...
    public static final Map<Integer, TranslationTextComponent> SPECIES_NAMES = ImmutableMap.<Integer, TranslationTextComponent>builder()
            .put(1, new TranslationTextComponent("message.creatures.greensawfish"))
            .put(2, new TranslationTextComponent("message.creatures.largetoothsawfish"))
            .put(3, new TranslationTextComponent("message.creatures.smalltoothsawfish"))
            .put(4, new TranslationTextComponent("message.creatures.dwarfsawfish"))
            .put(5, new TranslationTextComponent("message.creatures.narrowsawfish"))
            .build();
    public SawfishEntity(EntityType<? extends SawfishEntity> p_i50246_1_, World p_i50246_2_) {
        super(p_i50246_1_, p_i50246_2_);
        this.moveControl = new SawfishEntity.StingrayMoveHelperController(this);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event)
    {
        if (this.isMoving()) {
            if (this.isBaby()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("Swimbaby", true));
    ...
    public static AttributeModifierMap.MutableAttribute createAttributes() {
        return MobEntity.createMobAttributes().add(Attributes.MAX_HEALTH, 20.0D).add(Attributes.MOVEMENT_SPEED, 0.1D).add(Attributes.ATTACK_DAMAGE, 2.0D);
    }
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new SawfishEntity.HurtByTargetGoal(this));
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal<>(this, CodEntity.class, false));
    }
    ...
    static class StingrayMoveHelperController extends MoveHelperController {
        ...
        public void tick() {
            if (this.operation == Action.MOVE_TO && !this.fishbase.getNavigation().isDone()) {
                if ((this.wantedY - this.fishbase.getY())> 0) {
    ...
    static class StingrayRandomWalkingGoal extends RandomWalkingGoal {
        public StingrayRandomWalkingGoal(SawfishEntity p_i1648_1_, double p_i1648_2_) {
